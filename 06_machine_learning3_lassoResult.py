...
def main():
    ...
    # CSV一括予測 → 保存
    # =========================
    output_csv = "testL_processed2_Lasso_result.csv"
...
def predict_and_export_csv(pipe, df_all, output_path):
    """
    全行に対して
    - 予測坪単価
    - 相違率
    ...
    # モデル入力用Xを作る（学習時と同じ列・順序）
    X = df[USE_COLS].copy()

    # 予測（円/坪）
    df["PRED_PRICE_PER_TSUBO"] = pipe.predict(X)

    # 相違率（予測 - 実測）/ 実測
    df["DIFF_RATE"] = (
        df["PRED_PRICE_PER_TSUBO"] - df[TARGET]
    ) / df[TARGET]
    ...
    print(df[["PRICE_PER_TSUBO", "PRED_PRICE_PER_TSUBO", "DIFF_RATE"]].head())
